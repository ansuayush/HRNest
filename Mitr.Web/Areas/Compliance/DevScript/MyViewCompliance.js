...
function EditComplianceMaster(id) {
    ...
    CommonAjaxMethod(virtualPath + 'ComplianceTransaction/GetComplianceTransById', { id: id }, 'GET', function (response) {
        ...
        mdDate = complianceMaster.IsUpdated;
        ...
        if (StatusCode == 10 || StatusCode == 1 || StatusCode == 2) {
            BindDocData(uploadedDocument, 0);
            $("#btnSave").show();
            $("#btnSubmit").show();
        }
        else {
            ...
            $("#btnSave").hide();
            $("#btnSubmit").hide();
            if (StatusCode == 4 || StatusCode == 14 || StatusCode == 17) {
                $('#btnComplete').show();
                $('#btnCompleteSave').show();
                BindUL(subtask,2);
                BindDocData(uploadedDocument, 0);
            }
            else {
...
function addSubTaskRow() {
    ...
    var newRow = `
        <tr id="${rowId}">
            <td><input disabled type="text" id="${subTaskNameId}" value="" class="form-control Mandatory"  onchange="HideErrorMessage(this)"  Mandatory" placeholder="Enter" name="subTaskName[]">
             <span id="sp${subTaskNameId}" class="text-danger field-validation-error" style="display:none;">Hey, You missed this field!!</span>
            </td>
            <td>
                <select disabled class="form-control applyselect Mandatory"  onchange="HideErrorMessage(this)"  id="${responseTypeId}" name="responseType[]">
            ...
            <td>
                <div class="switch--box">
                    <label class="cswitch m-0">
                        <input disabled class="cswitch--input" id="${mandatoryId}" type="checkbox" name="mandatory[]"/><span class="cswitch--trigger wrapper"></span>
                    </label>
                </div>
            </td>
        ...
        if (from == 1) {
            if (vall.SubTaskType == "Numeric") {
                ...
                strData = '<li class="tasklsit">' +
                    ...
                    '<a ' + forFile + '  href="#" onclick="return DownloadTaskFile(' + vall.id + ')">Download <i class="fas fa-download float-right" data-toggle="tooltip" title="Download"></i></a>' +
            ...
            if (vall.SubTaskType == "Numeric") {
                strData = '<li class="tasklsit">' +
                    '<div class="taskcheck" >' +
                    '<input type="checkbox" disabled class="chkData" id="task' + vall.id + '" name="2" ' + chkData + ' ><label for="task' + vall.id + '"></label>' +
                    ' </div>' +
                    '<h2 class="tasktitle"> ' + vall.Subtask_Name + '</h2>' +
                    '<div class="task-contain"> ' +
                    '<div class="row"> ' +
                    ...
                    ' <div class="form-group col-sm-2"> ' +
                    '<label> Response Type <sup>' + mandateSign + '</sup ></label> ' +
                    '<input disabled type="number" class="form-control disableDetails ' + mandate + ' " id="txtTaskResponse_' + vall.id + '"  value="' + vall.SubTaskResponse1 + '" /> <span id="sptxtTaskResponse_' + vall.id + '" class="text-danger field-validation-error" style="display:none;">Hey, You missed this field!!</span>' +
                    '</div> ' +
                    '</div> ' +
                    '</div> ' +
                    '</li> ';
                ...
                strData = '<li class="tasklsit">' +
                    '<div class="taskcheck" >' +
                    '<input disabled type="checkbox" class="chkData" id="task' + vall.id + '" name="2" ' + chkData + '><label for="task' + vall.id + '"></label>' +
                    ' </div>' +
                    '<h2 class="tasktitle"> ' + vall.Subtask_Name + '</h2>' +
                    '<div class="task-contain"> ' +
                    '<div class="row"> ' +
                    ...
                    '<input disabled type="file" id="fileTaskDescription_' + vall.id + '" class="form-control disableDetails" placeholder="" onchange="UploadocumentTask(' + vall.id + ')">' +
                    '<input type="text" id="txtTaskFileName_' + vall.id + '"  class="form-control ' + mandate + '" style="display:none;" value="' + vall.SubTaskResponse1 + '"  />' +
                    '<span id="sptxtTaskFileName_' + vall.id + '" class="text-danger field-validation-error" style="display:none;">Hey, You missed this field!!</span>' +
                    '<input type="hidden" id="hdnTaskActualFileName_' + vall.id + '" value="' + vall.SubTaskResponse1 + '" />' +
                    '<input type="hidden" id="hdnTaskNewFileName_' + vall.id + '"  value="' + vall.SubTaskResponse2 + '"/>' +
                ...
                strData = '<li class="tasklsit">' +
                    ...
                    ' <div class="form-group col-sm-10"> ' +
                    '<label > Response Type <sup>' + mandateSign + '</sup ></label> ' +
                    '<textarea disabled class="form-control disableDetails ' + mandate + ' " id="txtTaskTextResponse_' + vall.id + '" placeholder = "Enter"  >' + vall.SubTaskResponse1 + '</textarea> <span id="sptxtTaskTextResponse_' + vall.id + '" class="text-danger field-validation-error" style="display:none;">Hey, You missed this field!!</span>' +
                    '</div> ' +
                    '</div> ' +
                    '</div> ' +
                    '</li> ';
