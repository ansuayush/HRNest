 $(document).ready(function () {
     BindOnboardUserOnboardPending();
});


function BindOnboardUserOnboardPending() {
    CommonAjaxMethod(virtualPath + 'OnboardingRequest/BindUserOnboard', { Id: 288, inputData: 13 }, 'GET', function (response) {
        ...
        $('#tblUserOnboardPending').html('');
        var newtbblData1 = '<table id="tblUserOnboardPending" class="fold-table table mt-0 " >' +
            ' <thead>' +
            ' <tr>' +
        ...
        $('#tblUserOnboardPending').html(newtbblData1 + tableData + html1);
    });
    HtmlPaging();
}
function BindOnboardUserCompleted() {
    CommonAjaxMethod(virtualPath + 'OnboardingRequest/BindUserOnboard', { Id: 288, inputData: 14 }, 'GET', function (response) {
        var mArray = response.data.data.Table;
        var statusRecord = response.data.data.Table1;
        if (statusRecord[0].Status == "14") {
            candodate_Status = true;
        ...
        $('#tblUserOnboardCompleted').html('');
        var newtbblData1 = '<table id="tblUserOnboardCompleted" class="fold-table table mt-0 " >' +
            ' <thead>' +
            ' <tr>' +
            ' <th class="tw-10">S.No</th>' +
            ' <th class="tw-150">Candidate Name</th>' +
            ...
            ' <th class="tw-150 ">Status</th>' +
            ' <th class="tw-100 " >Action</th>' +
            ' <th class="tw-100 " style="display:none">Map Emp</th>' +
            ' </tr>' +
            ' </thead>';
        var html1 = "</table>";
        var tableData = "";
        ...
        for (let i = 0; i < mArray.length; i++) {
            ...
            var newtbblData = "";
            if (mArray[i].IsMitrEmployee == false) {
                newtbblData = "<tr><td>" + mArray[i].RowNum + "</td><td>" + mArray[i].Candidate + "</td><td>" + mArray[i].PhoneNumber + "</td><td>" + mArray[i].EmailID + "</td><td>" + mArray[i].IssuesDOFLetter + "</td><td>" + OfferAcceptedDate + "</td><td>" + mArray[i].Status + "</td><td ><a  onclick=\"ViewApplication(" + mArray[i].REC_AppID + ");\"><i class=\"fas fa-eye\"></i>View</a><td style='display:none'><a  onclick=\"SubmitMasterEmployee(" + mArray[i].REC_AppID + ");\"><i class=\"fas fa-eye\"></i>View</a></td></tr>";
            }
            else {
                newtbblData = "<tr><td>" + mArray[i].RowNum + "</td><td>" + mArray[i].Candidate + "</td><td>" + mArray[i].PhoneNumber + "</td><td>" + mArray[i].EmailID + "</td><td>" + mArray[i].IssuesDOFLetter + "</td><td>" + OfferAcceptedDate + "</td><td>" + mArray[i].Status + "</td><td ><a  onclick=\"ViewApplication(" + mArray[i].REC_AppID + ");\"><i class=\"fas fa-eye\"></i>View</a><td style='display:none'><a  ><i class=\"fas fa-eye\" style=\"cursor:default!important\"></i>Mapped</a></td></tr>";
            }
            var allstring = newtbblData;
            tableData += allstring;
        }
        $('#tblUserOnboardCompleted').html(newtbblData1 + tableData + html1);
    });
    //HtmlPaging();
}
function HtmlPaging() {
