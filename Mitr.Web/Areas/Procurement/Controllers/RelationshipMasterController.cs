...
namespace Mitr.Areas.Procurement.Controllers
{
    public class RelationshipMasterController : BaseController
    {
        ...
        public RelationshipMasterController()
        {
            _objIProcurement = new ProcurementBAL();
        }
        [HttpGet]
        public JsonResult BindRelationshipMaster()
        {
            RelationshipMasterModel objSubCategoryMaster = new RelationshipMasterModel();
            objSubCategoryMaster.ID = Convert.ToInt32(Constants.RelationshipManager);
            ...
            try
            {
                ...
                var data = _objIProcurement.GetRecords(stringTOXml, Convert.ToString((int)Enum.Parse(typeof(Constants.ScreenIDProcurment), Constants.ScreenIDProcurment.RelationshipMaster.ToString())), roleId, userid, "GET", out errorMessage);
        ...
        [HttpGet]
        public JsonResult GetRelationshipMaster(int id)
        {
            RelationshipMasterModel objSubCategoryMaster = new RelationshipMasterModel();
        ...
        [HttpPost]
        public JsonResult SaveRelationshipMaster(RelationshipMasterModel objSubCategoryMaster)
        {
            objSubCategoryMaster.UserGrade = clsApplicationSetting.GetSessionValue("Grade");
            CommonMethods objCommonMethods = new CommonMethods();
            ...
            try
            {
                ...
                int roleId = 1;
                int userid = Convert.ToInt32(clsApplicationSetting.GetSessionValue("LoginID"));
                var data = _objIProcurement.PerformOperation(stringTOXml, Convert.ToString((int)Enum.Parse(typeof(Constants.ScreenIDProcurment), Constants.ScreenIDProcurment.RelationshipMaster.ToString())), roleId, userid, "GET", out errorMessage);
                string jsonData = JsonConvert.SerializeObject(data);
                return new JsonResult { Data = jsonData, JsonRequestBehavior = JsonRequestBehavior.AllowGet };
