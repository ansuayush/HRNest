...
{
    public class VenderTypeController : BaseController
    {
        // GET: Procurement/VenderType    

        IProcurement _objIProcurement;
        public VenderTypeController()
        {
            _objIProcurement = new ProcurementBAL();
        }
        [HttpGet]
        public JsonResult BindVenderType()
        {
            VendorTypeModel objSubCategoryMaster = new VendorTypeModel();
            objSubCategoryMaster.ID = Convert.ToInt32(Constants.VenderType);
            ...
            try
            {
                ...
                var data = _objIProcurement.GetRecords(stringTOXml, Convert.ToString((int)Enum.Parse(typeof(Constants.ScreenIDProcurment), Constants.ScreenIDProcurment.VendorType.ToString())), roleId, userid, "GET", out errorMessage);
        ...
        [HttpGet]
        public JsonResult GetVenderType(int id)
        {
            VendorTypeModel objSubCategoryMaster = new VendorTypeModel();
        ...
        [HttpPost]
        public JsonResult SaveVenderType(VendorTypeModel objSubCategoryMaster)
        {
            objSubCategoryMaster.UserGrade = clsApplicationSetting.GetSessionValue("Grade");
            CommonMethods objCommonMethods = new CommonMethods();
            ...
            try
            {
                ...
                int roleId = 1;
                int userid = Convert.ToInt32(clsApplicationSetting.GetSessionValue("LoginID"));
                var data = _objIProcurement.PerformOperation(stringTOXml, Convert.ToString((int)Enum.Parse(typeof(Constants.ScreenIDProcurment), Constants.ScreenIDProcurment.VendorType.ToString())), roleId, userid, "GET", out errorMessage);
                string jsonData = JsonConvert.SerializeObject(data);
                return new JsonResult { Data = jsonData, JsonRequestBehavior = JsonRequestBehavior.AllowGet };
