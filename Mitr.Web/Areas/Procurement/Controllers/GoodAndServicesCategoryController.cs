...
{
    public class GoodAndServicesCategoryController : BaseController
    {
        // GET: Procurement/GoodAndServicesCategory    

        IProcurement _objIProcurement;
        public GoodAndServicesCategoryController()
        {
            _objIProcurement = new ProcurementBAL();
        }
        [HttpGet]
        public JsonResult BindGoodAndServicesCategory()
        {
            GoodAndServicesCategoryModel objSubCategoryMaster = new GoodAndServicesCategoryModel();
            objSubCategoryMaster.ID = Convert.ToInt32(Constants.GoodAndServicesCategory);
            ...
            try
            {
                ...
                var data = _objIProcurement.GetRecords(stringTOXml, Convert.ToString((int)Enum.Parse(typeof(Constants.ScreenIDProcurment), Constants.ScreenIDProcurment.GoodAndServicesCategory.ToString())), roleId, userid, "GET", out errorMessage);
        ...
        [HttpGet]
        public JsonResult GetGoodAndServicesCategory(int id)
        {
            GoodAndServicesCategoryModel objSubCategoryMaster = new GoodAndServicesCategoryModel();
        ...
        [HttpPost]
        public JsonResult SaveGoodAndServicesCategory(GoodAndServicesCategoryModel objSubCategoryMaster)
        {
            objSubCategoryMaster.UserGrade = clsApplicationSetting.GetSessionValue("Grade");
            CommonMethods objCommonMethods = new CommonMethods();
            ...
            try
            {
                ...
                int roleId = 1;
                int userid = Convert.ToInt32(clsApplicationSetting.GetSessionValue("LoginID"));
                var data = _objIProcurement.PerformOperation(stringTOXml, Convert.ToString((int)Enum.Parse(typeof(Constants.ScreenIDProcurment), Constants.ScreenIDProcurment.GoodAndServicesCategory.ToString())), roleId, userid, "GET", out errorMessage);
                string jsonData = JsonConvert.SerializeObject(data);
                return new JsonResult { Data = jsonData, JsonRequestBehavior = JsonRequestBehavior.AllowGet };
