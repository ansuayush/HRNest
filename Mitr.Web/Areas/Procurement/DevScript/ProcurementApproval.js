    ...
    if (from == 1)
    {
        ...
        CommonAjaxMethod(virtualPath + 'ProcurementRequest/BindProjectDetails', { id: id, IsBindLine: 7 }
            , 'GET', function (response) {
                ...
                if (totalCount != 0 && totalCount == data2.length) {
                    $("#dvSubmit").hide();
                }
...
function LoadLineItemDataEd(Id, RId, d) {
    if (d == 1) {
        $("#hdnReqId").val(Id);
        $("#divApprove").show();
    }
    ...
    CommonAjaxMethod(virtualPath + 'ProcurementRequest/BindProjectDetails', { id: Id, IsBindLine: 8 }
        , 'GET', function (response) {
            ...
            $("#ddlAgreementAED").val(data1[0].ContractType).trigger('change');
            $("#ddlPOCAED").val(data1[0].POC).trigger('change');
            $("#lblTotal1AED").text(data1[0].BudgetAmount);
            $("#lblTotal2AED").text(data1[0].RequiredAmount);
            $('#ReqbyAED').val(data1[0].POCName);
            $('#ReqNoAED').val(data1[0].Req_No);
            $('#ReqDateAED').val(ChangeDateFormatToddMMYYY(data1[0].Req_Date));
            
            $('#userJustification').text(data1[0].Requiredjustificationforwaiver);

            $('#datatableApprovedED').DataTable({
                "processing": true, // for show progress bar           
                "destroy": true,
                "data": data2,
                "paging": false,
...
function ReturnLI(row, from) {
    ...
    else if (from == 4)//in process
    {
        ...
        if (row.StatusCode == 12 || row.StatusCode == 15 || row.StatusCode == 17 || row.StatusCode == 18 || row.StatusCode == 19 || row.StatusCode == 20 || row.StatusCode == 21 || row.StatusCode == 24 || row.StatusCode == 25 || row.StatusCode == 27 || row.StatusCode == 35) {
            ...
            if (row.StatusCode == 35 && row.Parent == 1) {
                ...
                if (row.StatusCode == 19 || row.StatusCode == 20 || row.StatusCode == 21 || row.StatusCode == 24 || row.StatusCode == 25 || row.StatusCode == 27) {
                    if (row.MessageSeen == 'Y')
                        moreInfo = '<a class="btn-c " onclick="AskQuery(' + row.Id + ',0,1)" data-toggle="modal" data-target="#mgs"><i class="fas fa-info-circle darkclr " data-toggle="tooltip" title="More Info"></i></a>';
                    else
                        moreInfo = '<a class="btn-c " onclick="AskQuery(' + row.Id + ',0,1)" data-toggle="modal" data-target="#mgs"><i class="fas fa-info-circle darkclr " data-toggle="tooltip" title="More Info"></i></a>';
                }
    ...
    else if (from == 6)//Close
    {
        ...
        else if (row.StatusCode == 9 || row.StatusCode == 10 || row.StatusCode == 11 || row.StatusCode == 12 || row.StatusCode == 13 || row.StatusCode == 14 || row.StatusCode == 15) {
...
function ReturnLIManager(row, from)
{
    var actionStr = ""
    if (from == 1)
    {
        //This manager pending data

        actionStr = '<a class="btn-g viewRequest" onclick="LoadLineItemDataPr(' + row.Id + ', ' + row.RequestId + ',1,' + row.Parent + ')" data-target="#pr" data-toggle="modal" title="View"><i class="fas fa-ellipsis-h"></i></a>';
    }
    else if (from == 2)
    {
        //This manager Approved data

        actionStr = '<a class="btn-g viewRequest" onclick="LoadLineItemDataPr(' + row.Id + ', ' + row.RequestId + ',2,' + row.Parent + ')" data-target="#pr" data-toggle="modal" title="View"><i class="fas fa-ellipsis-h"></i></a>';
    }
    else if (from == 3)
    {
        //This manager Resubmit data

        actionStr = '<a class="btn-g viewRequest" onclick="LoadLineItemDataPr(' + row.Id + ', ' + row.RequestId + ',3,' + row.Parent + ')" data-target="#pr" data-toggle="modal" title="View"><i class="fas fa-ellipsis-h"></i></a>';
    }
    else if (from == 4)
    {
        //This manager Rejected data

        actionStr = '<a class="btn-g viewRequest" onclick="LoadLineItemDataPr(' + row.Id + ', ' + row.RequestId + ',4,' + row.Parent + ')" data-target="#pr" data-toggle="modal" title="View"><i class="fas fa-ellipsis-h"></i></a>';
    }
    return '<li>' +
        '<div class="row d-flex" >' +
        '<div class="col-sm-4 align-self-center">' +
        '<div class="left-dtl-r"><strong>' + row.Req_ByName +'</strong><br>' +
        '<small class="lgt-text">Req#' + row.Req_No +
        ...
        '</div>' +
        '<div class="cntr-dtl-pst"> <strong class="trvlstatusclr">POC <br />' + row.POCName + '</strong> </div>' +
        '</div>' +
        '</div>' +
        '<div class="col-sm-3 align-self-center mb-flwdt">' +
        '<div class="right-dtl-r pt-0">' + actionStr +
...
function ReturnLIModuleAdminRFP(row, from)
{
    var actionStr = ""
    if (from == 1)
    {        
        actionStr = '<a class="btn-g viewRequest"  href="RFPProcessPending?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';
    }
    else if (from == 2)
    {
        //This manager Approved data
        ...
        if (row.ActualStatus == '11')
        {
            ...
            actionStr = '<a class="btn-g viewRequest"  href="ModuleAdminPendingProcessed?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';
...
function ReturnLIModuleAdminQA(row, from) {
    var actionStr = ""

    if (row.ContractType == 'Consultant') {
        if (row.DataFrom == 1) {
             actionStr = '<a class="btn-g viewRequest"  href="ModuleAdminQuotationConsultantRFP?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

        }
        else if (row.DataFrom == 2) {
             actionStr = '<a class="btn-g viewRequest"  href="ModuleAdminQuotationConsultant?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

        }
    }
    else if (row.ContractType == 'Sub-grant') {
        if (row.DataFrom == 1) {
            actionStr = '<a class="btn-g viewRequest"  href="ModuleAdminQuotationSubgrantRFP?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

         }
        else if (row.DataFrom == 2) {
             actionStr = '<a class="btn-g viewRequest"  href="ModuleAdminQuotationSubgrant?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

        }
    }
    else {
        if (row.DataFrom == 1) {
             actionStr = '<a class="btn-g viewRequest"  href="ModuleAdminQuotationRFP?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

        }
        else if (row.DataFrom == 2) {
             actionStr = '<a class="btn-g viewRequest"  href="ModuleAdminQuotation?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';
...
function ReturnLIED(row, from)
{
    ...
    if (row.DataFrom==0)
    {
        if (from == 1)
        {
            actionStr = '<a  href="#" onclick="ApprovedRejectDataED(9,' + row.Id + ')" class="btn-c" data-toggle="tooltip" title="Reject"><i class="fa fa-ban rct-clr"></i></a>' +
                '<a href="#" onclick="ApprovedRejectDataED(8,' + row.Id + ')" class="btn-a ApproveRequest" data-toggle="tooltip" title="Approved"><i class="fa fa-check"></i></a>' +
                '<a class="btn-g viewRequest" onclick="LoadLineItemDataEd(' + row.Id + ', ' + row.RequestId + ',1)" data-target="#pr-pED" data-toggle="modal" title="View"><i class="fas fa-ellipsis-h"></i></a>';
        }
        else
        {
           
            actionStr = '<a class="btn-g viewRequest" onclick="LoadLineItemDataEd(' + row.Id + ', ' + row.RequestId + ',3)" data-target="#pr-pED" data-toggle="modal" title="View"><i class="fas fa-ellipsis-h"></i></a>';
        ...
        if (row.ContractType == 'Consultant')
        {
            ...
            {
                if (from == 1) {
                    actionStr = '<a  href="#" onclick="ApproveRejectED(2,' + row.Id + ')" class="btn-c" data-toggle="tooltip" title="Reject"><i class="fa fa-ban rct-clr"></i></a>' +
                        '<a href="#" onclick="ApproveRejectED(1,' + row.Id + ')" class="btn-a ApproveRequest" data-toggle="tooltip" title="Approved"><i class="fa fa-check"></i></a>' +
                        '<a class="btn-g viewRequest"  href="AuthorisedSignatoryQuotationConsultantRFP?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

                }
                else {
                    actionStr = '<a class="btn-g viewRequest"  href="AuthorisedSignatoryQuotationConsultantRFP?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';
            ...
            else if (row.DataFrom == 2)
            {
                ...
                if (from == 1) {
                    actionStr = '<a  href="#" onclick="ApproveRejectED(2,' + row.Id + ')" class="btn-c" data-toggle="tooltip" title="Reject"><i class="fa fa-ban rct-clr"></i></a>' +
                        '<a href="#" onclick="ApproveRejectED(1,' + row.Id + ')" class="btn-a ApproveRequest" data-toggle="tooltip" title="Approved"><i class="fa fa-check"></i></a>' +
                        '<a class="btn-g viewRequest"  href="AuthorisedSignatoryQuotationConsultant?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

                }
                else {
                    actionStr = '<a class="btn-g viewRequest"  href="AuthorisedSignatoryQuotationConsultant?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';
        ...
        else if (row.ContractType == 'Sub-grant')
        {
            if (row.DataFrom == 1)
            {
                ...
                if (from == 1) {
                    actionStr = '<a  href="#" onclick="ApproveRejectED(2,' + row.Id + ')" class="btn-c" data-toggle="tooltip" title="Reject"><i class="fa fa-ban rct-clr"></i></a>' +
                        '<a href="#" onclick="ApproveRejectED(1,' + row.Id + ')" class="btn-a ApproveRequest" data-toggle="tooltip" title="Approved"><i class="fa fa-check"></i></a>' +
                        '<a class="btn-g viewRequest"  href="AuthorisedSignatoryQuotationSubgrantRFP?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

                }
                else {
                    actionStr = '<a class="btn-g viewRequest"  href="AuthorisedSignatoryQuotationSubgrantRFP?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';
            ...
            else if (row.DataFrom == 2)
            {
                if (from == 1) {
                    actionStr = '<a  href="#" onclick="ApproveRejectED(2,' + row.Id + ')" class="btn-c" data-toggle="tooltip" title="Reject"><i class="fa fa-ban rct-clr"></i></a>' +
                        '<a href="#" onclick="ApproveRejectED(1,' + row.Id + ')" class="btn-a ApproveRequest" data-toggle="tooltip" title="Approved"><i class="fa fa-check"></i></a>' +
                        '<a class="btn-g viewRequest"  href="AuthorisedSignatoryQuotationSubgrant?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

                }
                else {
                    actionStr = '<a class="btn-g viewRequest"  href="AuthorisedSignatoryQuotationSubgrant?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';
            ...
            if (row.DataFrom == 1)
            {
                ...
                if (from == 1) {
                    actionStr = '<a  href="#" onclick="ApproveRejectED(2,' + row.Id + ')" class="btn-c" data-toggle="tooltip" title="Reject"><i class="fa fa-ban rct-clr"></i></a>' +
                        '<a href="#" onclick="ApproveRejectED(1,' + row.Id + ')" class="btn-a ApproveRequest" data-toggle="tooltip" title="Approved"><i class="fa fa-check"></i></a>' +
                        '<a class="btn-g viewRequest"  href="AuthorisedSignatoryQuotationRFP?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

                }
                else {
                    actionStr =  '<a class="btn-g viewRequest"  href="AuthorisedSignatoryQuotationRFP?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';
            ...
            else if (row.DataFrom == 2)
            {
                ...
                if (from == 1) {
                    actionStr = '<a  href="#" onclick="ApproveRejectED(2,' + row.Id + ')" class="btn-c" data-toggle="tooltip" title="Reject"><i class="fa fa-ban rct-clr"></i></a>' +
                        '<a href="#" onclick="ApproveRejectED(1,' + row.Id + ')" class="btn-a ApproveRequest" data-toggle="tooltip" title="Approved"><i class="fa fa-check"></i></a>' +
                        '<a class="btn-g viewRequest"  href="AuthorisedSignatoryQuotation?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

                }
                else {
                    actionStr =  '<a class="btn-g viewRequest"  href="AuthorisedSignatoryQuotation?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';
...
function ReturnLICommiteeMember(row, from)
{
    var actionStr = ""
    if (row.MessageSeen == 'Y')
        var moreInfo = '<a class="btn-c " onclick="AskQuery(' + row.Id + ',1,1)" data-toggle="modal" data-target="#mgs"><i class="fas fa-plus green-clr" data-toggle="tooltip" title="Ask a Query"   data-original-title="Ask Query"></i></a>';
    else
        var moreInfo = '<a class="btn-c " onclick="AskQuery(' + row.Id + ',1,1)" data-toggle="modal" data-target="#mgs"><i class="fas fa-plus darkclrNewMsg" data-toggle="tooltip" title="Ask a Query"   data-original-title="Ask Query"></i></a>';


    if (row.ContractType == 'Consultant') {
        if (row.DataFrom == 1) {
            actionStr = moreInfo+ '<a class="btn-g viewRequest"  href="ProcurementCommitteeQuotationConsultantRFP?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

        }
        else if (row.DataFrom == 2) {
            actionStr = moreInfo + '<a class="btn-g viewRequest"  href="ProcurementCommitteeQuotationConsultant?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

        }
    }
    else if (row.ContractType == 'Sub-grant') {
        if (row.DataFrom == 1) {
            actionStr = moreInfo + '<a class="btn-g viewRequest"  href="ProcurementCommitteeQuotationSubgrantRFP?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

        }
        else if (row.DataFrom == 2) {
            actionStr = moreInfo + '<a class="btn-g viewRequest"  href="ProcurementCommitteeQuotationSubgrant?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

        }
    }
    else {
        if (row.DataFrom == 1) {
            actionStr = moreInfo + '<a class="btn-g viewRequest"  href="ProcurementCommitteeQuotationRFP?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';

        }
        else if (row.DataFrom == 2) {
            actionStr = moreInfo + '<a class="btn-g viewRequest"  href="ProcurementCommitteeQuotation?id=' + row.Id + '" title="View"><i class="fas fa-ellipsis-h"></i></a>';
...
function BindProcureRequest()
{
    ...
    CommonAjaxMethod(virtualPath + 'ProcurementRequest/BindProcureRequest', null, 'GET', function (response) {
        ...
        var dataPMPending = response.data.data.Table6;//user request Pending
        var dataPMApproved = response.data.data.Table7;//user request Approved
        var dataPMResubmit = response.data.data.Table8;//user request Resubmit
        var dataPMRejected = response.data.data.Table9;//user request Rejected


        var dataModuleAdminRFPPending = response.data.data.Table10;//user request  ModuleAdminRFP Pending
        var dataModuleAdminRFPProcess = response.data.data.Table11;//user request ModuleAdminRFP Approved
        var dataModuleAdminRFPLive = response.data.data.Table12;//user request ModuleAdminRFP Live
        var dataModuleAdminRFPRejected = response.data.data.Table13;//user request ModuleAdminRFP Rejected

        var dataModuleAdminQAPending = response.data.data.Table14;//user request  dataModuleAdminQA Pending
        var dataModuleAdminQAProcess = response.data.data.Table15;//user request dataModuleAdminQA In Process
        var dataModuleAdminQARejected = response.data.data.Table16;//user request dataModuleAdminQA Rejected
        var dataModuleAdminQALive = response.data.data.Table17;//user request dataModuleAdminQA Live


        var dataCommiteePending = response.data.data.Table18;//Commitee data Pending 
        var dataCommiteeApproved = response.data.data.Table19;//Commitee data Apporved 
        var dataCommiteeRejected = response.data.data.Table20;//Commitee data Rejected 

        var dataEDPending = response.data.data.Table21;//ED data Pending 
        var dataEDApproved = response.data.data.Table22;//ED data Apporved 
        var dataEDRejected = response.data.data.Table23;//ED data Rejected
        ...
        {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';            
            $("#ulPMTabPending").append(d);
        }
        ...
        if (dataArroved.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulPMTabApproved").append(d);
        }
        ...
        if (dataRejected.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulPMTabRejected").append(d);
        }
        ...
        for (var i = 0; i < dataPMPending.length; i++) {
            var liData = ReturnLIManager(dataPMPending[i], 1);
            $("#ulUserRequestPending").append(liData);
        }
        if (dataPMPending.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulUserRequestPending").append(d);
        }

        for (var i = 0; i < dataPMApproved.length; i++) {
            var liData = ReturnLIManager(dataPMApproved[i], 2);
            $("#ulUserRequestApproved").append(liData);
        }
        if (dataPMApproved.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulUserRequestApproved").append(d);
        }
        for (var i = 0; i < dataPMResubmit.length; i++) {
            var liData = ReturnLIManager(dataPMResubmit[i], 3);
            $("#ulUserRequestResubmit").append(liData);
        }
        if (dataPMResubmit.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulUserRequestResubmit").append(d);
        }
        for (var i = 0; i < dataPMRejected.length; i++) {
            var liData = ReturnLIManager(dataPMRejected[i], 4);
            $("#ulUserRequestReject").append(liData);
        }
        if (dataPMRejected.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulUserRequestReject").append(d);
        }
        ...
        for (var i = 0; i < dataModuleAdminRFPPending.length; i++) {
            $("#divModuleApproval").show();
            
            var liData = ReturnLIModuleAdminRFP(dataModuleAdminRFPPending[i], 1);
            $("#ulMARFPPending").append(liData);
        }
        if (dataModuleAdminRFPPending.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulMARFPPending").append(d);
        }
        //Process
        for (var i = 0; i < dataModuleAdminRFPProcess.length; i++) {
            var liData = ReturnLIModuleAdminRFP(dataModuleAdminRFPProcess[i], 2);
            $("#ulMARFPProcess").append(liData);
        }
        if (dataModuleAdminRFPProcess.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulMARFPProcess").append(d);
        }
        //Live
        for (var i = 0; i < dataModuleAdminRFPLive.length; i++) {
            var liData = ReturnLIModuleAdminRFP(dataModuleAdminRFPLive[i], 3);
            $("#ulMARFPLive").append(liData);
        }
        if (dataModuleAdminRFPLive.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulMARFPLive").append(d);
        }
        //Rejected
        for (var i = 0; i < dataModuleAdminRFPRejected.length; i++) {
            var liData = ReturnLIModuleAdminRFP(dataModuleAdminRFPRejected[i], 4);
            $("#ulMARFPRejected").append(liData);
        }
        if (dataModuleAdminRFPRejected.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulMARFPRejected").append(d);
        }
        //Module Admin Quotation

        //Pending
        for (var i = 0; i < dataModuleAdminQAPending.length; i++)
        {
            $("#idivModuleMember").show();
            $("#iModuleMember").hide();
            var liData = ReturnLIModuleAdminQA(dataModuleAdminQAPending[i], 1);
            $("#ulMAQAPending").append(liData);
        }
        if (dataModuleAdminQAPending.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulMAQAPending").append(d);
        }
        //Process
        for (var i = 0; i < dataModuleAdminQAProcess.length; i++) {
            var liData = ReturnLIModuleAdminQA(dataModuleAdminQAProcess[i], 2);
            $("#ulMAQAProcess").append(liData);
        }
        if (dataModuleAdminQAProcess.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulMAQAProcess").append(d);
        }
       //Live
        
        for (var i = 0; i < dataModuleAdminQALive.length; i++) {
            var liData = ReturnLIModuleAdminQA(dataModuleAdminQALive[i], 3);
            $("#ulMAQALive").append(liData);
        }
        if (dataModuleAdminQALive.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulMAQALive").append(d);
        }

       //Rejected
        for (var i = 0; i < dataModuleAdminQARejected.length; i++) {
            var liData = ReturnLIModuleAdminQA(dataModuleAdminQARejected[i], 4);
            $("#ulMAQARejected").append(liData);
        }

        if (dataModuleAdminQARejected.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulMAQARejected").append(d);
        }
        ...
        for (var i = 0; i < dataCommiteePending.length; i++)
        {
            $("#divPCApproval").show();
          
            
            var liData = ReturnLICommiteeMember(dataCommiteePending[i], 1);
            $("#ulPCPending").append(liData);
        }
        if (dataCommiteePending.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulPCPending").append(d);
        }
        //Approved
        for (var i = 0; i < dataCommiteeApproved.length; i++) {
            var liData = ReturnLICommiteeMember(dataCommiteeApproved[i], 2);
            $("#ulPCApproved").append(liData);
        }
        if (dataCommiteeApproved.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulPCApproved").append(d);
        }
        //Rejected
        for (var i = 0; i < dataCommiteeRejected.length; i++) {
            var liData = ReturnLICommiteeMember(dataCommiteeRejected[i], 3);
            $("#ulPCRejected").append(liData);
        }
        if (dataCommiteeRejected.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulPCRejected").append(d);
        }
        ...
        for (var i = 0; i < dataEDPending.length; i++) {
            $("#divEDApproval").show();
            
            var liData = ReturnLIED(dataEDPending[i], 1);
            $("#ulEDPending").append(liData);
        }
        if (dataEDPending.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulEDPending").append(d);
        }

        //Approved
        for (var i = 0; i < dataEDApproved.length; i++) {
            var liData = ReturnLIED(dataEDApproved[i], 2);
            $("#ulEDApproved").append(liData);
        }

        if (dataEDApproved.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulEDApproved").append(d);
        }
        //Rejected
        for (var i = 0; i < dataEDRejected.length; i++) {
            var liData = ReturnLIED(dataEDRejected[i], 3);
            $("#ulEDRejected").append(liData);
        }
        if (dataEDRejected.length == 0) {
            var d = '<div class="no-dat-dv"><img src="../assets/design/images/not-found-image.png" alt="No-Image"><p class="mtr-o-clr">No Record Found</p></div>';
            $("#ulEDRejected").append(d);
        }
...
function GetQuotationContract2(id, requestId, from,isAmend) {
    CommonAjaxMethod(virtualPath + 'ProcurementRequest/BindProjectDetails', { id: id, IsBindLine: 3 }
        , 'GET', function (response) {
            $('#hdnProcureId').val(id);
            var data4 = response.data.data.Table5;
            ...
            },
            {
                ...
                data: null, render: function (data, type, row) {
                    if (row.InvoiceNo != '')
                        ...
                        return '<input type="text" alt=""   onchange="HideErrorMessage(this)" onkeypress="validate(event)" onpaste="validate(event)" class="form-control Finance" placeholder="Enter" id="tbInvoice_' + row.Id + '"> <span id="sptbInvoice_' + row.Id + '" class="text-danger field-validation-error" style="display:none;">Hey, You missed this field!!</span>';
                    }
                }
            },
            {
...
function Bindtbldod3(array, isAmend) {
...
function BindAmendmentDataPM(ReqId, parent) {
    CommonAjaxMethod(virtualPath + 'ProcurementRequest/BindProjectDetails', { id: ReqId, IsBindLine: 9 }
        , 'GET', function (response) {
            var data1 = response.data.data.Table;
            $('#txtAmendReason').val('');
...
function ChangeAmendmentDetailsPr(ctrl)
{
    var Id = ctrl.value;
    var Status = $('#ddlAmendmentVersion option[value="' + $(ctrl).val() + '"]').attr("dataEle")
    var statudData = Status.split('-');
    var StatusCode = statudData[0];
    var ContractType = statudData[1];
    var DataFrom = statudData[2];
    var Parent = statudData[3];
    if (ContractType == "1") {
        ContractType = "Consultant";
    }
    else if (ContractType == "2") {
        ContractType = "Sub-grant";
    ...
    LoadLineItemOnchange(Id, StatusCode, 1,  Parent);
}
function LoadLineItemOnchange(id, reqId, from, parent) {
    rowId = 0;     
    $("#hdnProcureRequestId").val(id);
    if (from != 1) {
        $("#chkAll").hide();
    ...
    CommonAjaxMethod(virtualPath + 'ProcurementRequest/BindProjectDetails', { id: id, IsBindLine: 5 }
        , 'GET', function (response) {
            ...
            var data2 = response.data.data.Table1;
            // var data3 = response.data.data.Table2;
            $("#ddlAgreementPr").val(data1[0].ContractType).trigger('change');
            $("#ddlPOCPr").val(data1[0].POC).trigger('change');
            $("#lblTotal1Pr").text(data1[0].BudgetAmount);
            $("#lblTotal2Pr").text(data1[0].RequiredAmount);
            $('#ReqbyPr').val(data1[0].POCName);
            $('#ReqNoPr').val(data1[0].Req_No);
            $('#ReqDatePr').val(ChangeDateFormatToddMMYYY(data1[0].Req_Date));

            $('#datatableLineItemPr').DataTable({
                "processing": true, // for show progress bar           
                "destroy": true,
                "data": data2,
                "paging": false,
                ...
                "columns": [
...

                    {
                        ...
                        data: null, render: function (data, type, row) {
                            ...
                            var strReturn = "";
                            if (parent == 1) {
                                strReturn = '<div class="icon-thread">' +
...

                    {
                        ...
                        data: null, render: function (data, type, row) {
                            ...
                            var strReturn = '<input type="checkbox" id="chk-' + row.Id + '-' + row.Procure_Request_Id + '" class="selectedId1 sltchk1" name="select" onchange="valueChanged1()"><label for= "chk-' + row.Id + '-' + row.Procure_Request_Id + '" class= "m-0" ></label>';
                            if (from == 1) {
                                if (row.IsManager == 1 && row.Status == 'Pending') {
                                    return strReturn;
                                }
...
function LoadLineItemDataPr(id, reqId, from,parent) {
    rowId = 0;
    BindAmendmentDataPM(id, parent);
    $("#hdnProcureRequestId").val(id);
    if (from != 1) {
        $("#chkAll").hide();
        $("#divApproval").hide();
...
function ApproveRejectRequestPM()
{
    $('#confirmmsgrcmultiple').modal('show');

}
 
function ApprovedRejectDataED(from,reId) {
    var msg = "Pls Reconfirm";
    if (from == 8) {
        msg = "Pls Reconfirm";
    }
...
function AppoveRequest(from) {
    if (from == 2) {
        ...
        if (RequestApprovalArray.length > 0)
        {
            CommonAjaxMethod(virtualPath + 'ProcurementRequest/ApproveRejectProcurementRegistration', RequestApprovalArray, 'POST', function (response) {
                window.location.reload();
            });
...
function ApproveRejectED(from, reqID) {
...
function SendMessageToUser(ctrl) {
    var moreInfoId= $("#hdMoreInfoId").val();
    var id = ctrl.id.split('_');
    var controlNo = id[1];
    if (checkValidationOnSubmit('Message_' + controlNo) == true) {
        ...
        CommonAjaxMethod(virtualPath + 'ProcurementRequest/SaveRFPEntry', obj, 'POST', function (response) {
            AskQuery(moreInfoId, $('#hdnIsModuleAdminMessage').val());
        });
    }
}
function AskQuery(id, from,isview) {
    ...
    $('#hdnIsModuleAdminMessage').val(from);
    $("#hdMoreInfoId").val(id);
