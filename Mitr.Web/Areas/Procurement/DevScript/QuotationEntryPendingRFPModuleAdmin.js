...
function BindProcureRequest(id)
{
    var reqId = RequestId;
    if (id != undefined) {
        reqId = id;
        ...
        "columns": [
           ...
           
            {
                ...
                data: null, render: function (data, type, row) {
                    ...
                    var strReturn = '<a href="#" onclick="DownloadPublishFile(' + row .Id+')" ><i class="fas fa-paperclip"></i>' + row.AttachmentActualName + '</a><input value="' + row.AttachmentActualName + '" type="hidden" id="hdnPublishAName_' + row.Id + '" /><input value="' + row.AttachmentPath + '" type="hidden" id="hdnPublishUrl_' + row.Id +'" />';
                    return strReturn;
                }
            },
            { "data": "Remarks" } 
...
function RedirectToAgreement(ctr) {
    ...
    if (ddlValue == 'Consultant') {
        btn.href = "/Procurement/QuotationEntryPendingConsultantRFP?id=" + RequestId;
        btn.click();
    }
...
function FillTax() {
    ...
    var total1 = (gstamt1);
    $('#txtTax_GST').val(Math.round(total1));
...
function FillUnitTax()
{
    ...
    var gstamt1 = (tCal1 * parseInt(txtUnitTaxPer))/100;     
...
function GetQuotationEntry(id) {
    var reqId = RequestId;
    if (id != undefined) {
        reqId = id;
    }
    CommonAjaxMethod(virtualPath + 'ProcurementRequest/GetQuotationEntry', { Id: reqId }
        , 'GET', function (response) {
        ...
        var data5 = response.data.data.Table4;//Procure_QuotationRateContractMultiple
            ...
            if (data1.length > 0) {
                ...
                $('#txtDeliveryAddress').val(data1[0].DeliveryAddress);
                    ...
                    $('#txtAmendmentReason').text(data1[0].AmendmentReason);
               ...
               // $("#ddlAgreementA").val('ddlAgreementA').trigger('change');
...
function BindContract(objarrayinner) {       
    ...
    UploadSignedDocuments = objarrayinner;
    for (var i = 0; i < objarrayinner.length; i++) {
...
function GetQuotationMember(id) {

    var reqId = RequestId;
    if (id != undefined) {
        reqId = id;
    }
    CommonAjaxMethod(virtualPath + 'ProcurementRequest/BindProjectDetails', { id: reqId, IsBindLine: 15 }
        , 'GET', function (response) {
        var data1 = response.data.data.Table;
...
function GetQuotationMessage(id) {
    var reqId = RequestId;
    if (id != undefined) {
        reqId = id;
    }
    CommonAjaxMethod(virtualPath + 'ProcurementRequest/BindProjectDetails', { id: reqId, IsBindLine: 16 }
        , 'GET', function (response) {
        var data1 = response.data.data.Table;
