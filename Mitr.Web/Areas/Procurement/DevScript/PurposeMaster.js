...
function SavePurposeMaster()
{
 ;
///This is for testing for merhing branch
    if (checkValidationOnSubmit('Mandatory') == true)
    {
        var catId = '';        
        var obj = {
            ID: catId,
            Purpose: $('#txtSubCate').val(),
            TableType: DropDownTypeEnum.ProcurementPurpose,
            Code: "SC",
            MasterTableId: $('#hdnMasterTableId').val(),
            UserId: loggedinUserid,
            IPAddress: $('#hdnIP').val() ? "" : " "   
        }
        CommonAjaxMethod(virtualPath + 'PurposeMaster/SavePurposeMaster', obj, 'POST', function (response)
        {
            BindPurposeMaster();
            ClearFormControl();       
            $('#sc').modal('hide')
...
function BindPurposeMaster()
{
    CommonAjaxMethod(virtualPath + 'PurposeMaster/BindPurposeMaster', null, 'GET', function (response) {
...
{
    ClearFormControl();
    CommonAjaxMethod(virtualPath + 'PurposeMaster/GetPurposeMaster', { id: id }, 'GET', function (response)
    {  
