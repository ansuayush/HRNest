    ...
    if (isValidJust) {
        ...
        if (checkValidationOnSubmit('Mandatory') == true && checkDate3 == true && isValiddeliverables == true && isValidReimbursable == true && isValidfileScopeNewName == true && isLineItemdocumentUpload == true && isValidfileBudgetNewName == true && isValidIsRecommend == true) {
            if (isValid > 0) {
                ...
                FailToaster("Please select/fill Quotation Entry line item!");
            }
        }
    }
}
        ...
        "columns": [
...

            {
                ...
                data: null, render: function (data, type, row) {
                    ...
                    return '<label class="text-right" id="lblInvoiceAmount2_' + row.Id + '">' + NumberWithComma(row.InvoiceAmount) + '</label>';
...

            {
                ...
                data: null, render: function (data, type, row) {
                    ...
                    return '<label   style="display:none;" id="hdnUploadFileUrlPD2_' + row.Id + '" >' + row.InvoiceAttachmentURL + '</label >' +
                        ' <label style="display:none;"  id="hdnUploadActualFileNamePD2_' + row.Id + '">' + row.InvoiceAttachmentActualName + '</label>' +
                        '<a id="ancUploadActualFileNamePD2_' + row.Id + '" href="" onclick="DownloadFileQuotationPD2(this)">Download <i class="fas fa-download float-right" data-toggle="tooltip" title="Download"></i></a>' +
                ...
                data: null, render: function (data, type, row) {
                    ...
                    if (row.InvoiceAttachmentActualNameD != undefined) {
                        return '<label   style="display:none;" id="hdnUploadFileUrlD1_' + row.Id + '" >' + row.InvoiceAttachmentURLD + '</label >' +
                            ' <label style="display:none;"  id="hdnUploadActualFileNameD1_' + row.Id + '">' + row.InvoiceAttachmentActualNameD + '</label>' +
                            '<a id="ancUploadActualFileNameD1_' + row.Id + '" href="" onclick="DownloadFileQuotationD1(this)">Download <i class="fas fa-download float-right" data-toggle="tooltip" title="Download"></i></a>' +
            ...
            },
            {
                ...
                data: null, render: function (data, type, row) {
                    ...
                    if (fStatus == 'Pending') {
                        return '<input type="text" alt=""   onkeypress="validate(event)" onpaste="validate(event)" class="form-control Finance2" placeholder="Enter" id="tbTDSAmount2_' + row.Id + '" value="' + row.TDS +'"> <span id="sptbTDSAmount2_' + row.Id + '" class="text-danger field-validation-error" style="display:none;">Hey, You missed this field!!</span>';
        ...
        "columns": [
...

            {
                ...
                data: null, render: function (data, type, row) {
                    ...
                    return '<label class="text-right" id="lblInvoiceAmount3_' + row.Id + '">' + NumberWithComma(row.InvoiceAmount) + '</label>';
            ...
            },
            {
                ...
                data: null, render: function (data, type, row) {
                    ...
                    if (fStatus == 'Pending') {
                        return '<input type="text" alt=""   onkeypress="validate(event)" onpaste="validate(event)" class="form-control Finance2" placeholder="Enter" id="tbTDSAmount3_' + row.Id + '" value="' + row.TDS +'"> <span id="sptbTDSAmount3_' + row.Id + '" class="text-danger field-validation-error" style="display:none;">Hey, You missed this field!!</span>';
...
function InvoiceApprove(ctrl, from, type) {
    ...
    if ($('#lblInvoiceAmount' + from + '_' + controlNo).text() != "") {
        lblInvoiceAmount_ = parseInt($('#lblInvoiceAmount' + from + '_' + controlNo).text().replace(/,/g, ''));
