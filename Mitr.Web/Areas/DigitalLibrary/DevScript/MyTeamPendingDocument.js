...
function EditContent(id, RequestId)
{
    $("#submitView").show();
    $("#submitAdd").hide();
    $("#DivDocumentListUpdate").hide();
...
{
    CommonAjaxMethod(virtualPath + 'DigitalLibrary/GetMyContentForApproval',  null, 'GET', function (response)
    {
        ...
        $('#tblPendingContent').DataTable({
            ...
            "columns": [
                    ...
                    data: null, render: function (data, type, row)
                    {
                        ...
                        var strReturn = '<input type="checkbox" id="chk-' + row.Id + '-' + row.RequestId + '" class="selectedId sltchk" name="select" onchange="valueChanged()"><label for= "chk-' + row.Id + '-' + row.RequestId +'" class= "m-0" ></label>';
                ...
                
                {
                    ...
                    data: null, render: function (data, type, row)
                    {
                        ...
                        var strReturn = '<a href="#" onclick="EditContent(' + row.Id + ', ' + row.RequestId + ')"><i class="fas fa-edit"></i>View</a>';
                        return strReturn;
                    }
                }
...
function SaveData(isAprroved)
{
    ...
    if ($('#hdnType').val() == 'Div3')
    {
        ...
        var obj =
        {
            ...
            IPAddress: $('#hdnIP').val(),
            UserGrade: UserGrade,
            Reason: extactValueRejectReason
        }
        if (isValid)
        {
            contentArrayToApproval.push(obj);
            CommonAjaxMethod(virtualPath + 'DigitalLibrary/DLContentApproval', contentArrayToApproval, 'POST', function (response) {
                BindContent();
