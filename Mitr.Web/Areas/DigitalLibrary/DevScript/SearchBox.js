            ...
            "columns": [
                ...
                { "data": "FileSize" },
                {
                    ...
                    data: null, render: function (data, type, row) {
                        ...
                        var strReturn = '<a href="' + row.FileUrl +'" target="_blank" class="green-clr"><i class="fas fa-download"></i>Download</a><span class="divline">';
                        return strReturn;
                    }
                }
...
{
    if (jQuery.trim($('#txtSearchBox').val()) == "" && $('#ddlCategory').val() == "Choose Your Category" && ($('#ddlSubCategory').val() == "Select" || $('#ddlSubCategory').val()==null))
    {
    ...
    else
    {
        ...
        CommonAjaxMethod(virtualPath + 'DigitalLibrary/GetSearchContent', SearchObject, 'POST', function (response)
        {
            ...
            for (var i = 0; i < dt.length; i++)
            {
                ...
                {
                    downloadprintshare = '<a href="#" onclick="ShowDownload(' + dt[i].ID + ')" data-toggle="modal" data-target="#dwn"><i class="fas fa-eye"></i>View</a><span class="divline">|</span><a href="#" onclick="ShareContent(' + dt[i].ID + ')" ><i class="fas fa-share"></i>Share</a>';
                }
                else if (dt[i].IsApproved == 3)
                {
                    downloadprintshare = '<a href="#"  onclick="RequestForDownload(' + dt[i].ID + ',' + dt[i].ManagerID + ')"  class="red-clr"><i class="fas fa-download"></i>Request for Download</a>';
                }
                else if (dt[i].IsApproved == 0)
                {
                    downloadprintshare = '<i class="fas fa-clock"></i>Request pending for Download';

                }
                else if (dt[i].IsApproved == 2) {
                    downloadprintshare = '<i class="fas fa-clock"></i>Rejected';
                ...
                var li =
                    ...
                    '<span class="d-block">' + dt[i].Year + ' </span>' +
