...
var mentorsArray = []
function ClubbedAttendeesList() {
    CommonAjaxMethod(virtualPath + 'HRRequest/GetHRTrainingDetails', { RequIds: ReqIDs, inputData: 1 }, 'GET', function (response) {
        attendeesArray = response.data.data.Table;
        dataArry = response.data.data.Table1;
        trainingCalandarArray = response.data.data.Table2;
        ...
        var newtbblData1 = '<table class="table mt-0 " >' +
            ...
            ' <th width="50" style="display:none">UserTrainingID</th>' +
            ' </tr>' +
            ' </thead>';
        var html1 = "</table>"; 
        ...
        for (let i = 0; i < attendeesArray.length; i++) {
            var AttendeesTypeName = attendeesArray[i].AttendeesType == 148 ? 'Mandatory' : attendeesArray[i].AttendeesType == 149 ? 'Optional' : '';
            var RequestID = 'CC-' + attendeesArray[i].ReqID;
            attendeesArray[i].RowNum = parseInt(i + 1)
            var newtbblData = "<tr><td>" + attendeesArray[i].RowNum + "</td><td>" + attendeesArray[i].AttendeesNames + "</td><td>" + AttendeesTypeName + "</td><td>" + attendeesArray[i].RequestSource + " </td><td><a href='#' data-toggle='modal' data-target='#RequestDetailsModal' onclick=\"ViewRequestDetails('" + attendeesArray[i].UserTrainingID + "')\"><i class='fas fa - eye'></i>" + attendeesArray[i].RequestName + "</a> </td><td style='display: none'>" + attendeesArray[i].AttendeesType + "</td><td></td><td style='display:none'>" + attendeesArray[i].AttendEmployeeID + "</td><td style='display:none'>" + attendeesArray[i].UserTrainingID + "</td></tr>"; 
            var allstring = newtbblData;
            tableData += allstring;
        }
        $('#tblAddAttendeesList').html(newtbblData1 + tableData + html1);
        //Popup
        ...
        //    TrainingTypeID = trainingCalandarArray[0].TrainingTypeid
        //    TrainingName = trainingCalandarArray[0].TrainingType
        //}
        $('#tblSelectTrainingCalendarList').html('');
        var newtbblDataScreen = '<table class="table mt-2 " >' +
            ' <thead>' +
            ' <tr>' +
            ' <th width="33">S.No</th>' +
            ...
            ' <th width="300" ><span id="spLocationAndLink">Location</span></th>' +
            ' <th style="display:none"></th>' +
            ' </tr>' +
            ' </thead>';
        var htmlScreen = "</table>";
        var tableDataScreen = "";
        var i = 0;
        ...
        if (trainingCalandarArray.length > 0) {
            for (i; i < trainingCalandarArray.length; i++) {
                if (i == 0) {
                    TrainingName = trainingCalandarArray[0].TrainingName
                    TrainingTypeID = trainingCalandarArray[0].TrainingTypeid
                    trainingModeType = trainingCalandarArray[i].TrainingMode;
        ...
        $('#tblSelectTrainingCalendarList').html(newtbblDataScreen + tableDataScreen + htmlScreen);
        if (trainingCalandarArray.length > 0) {
            for (i; i < trainingCalandarArray.length; i++) {
                if (i == 0) {
...
function ViewRequestDetails(reqId) {
    CommonAjaxMethod(virtualPath + 'HRRequest/GetHRTrainingDetails', { RequIds: reqId, inputData: 6 }, 'GET', function (response) {
        ...
        if (ReqDetails.length > 0) {
            ...
            document.getElementById('lblRequestName').innerHTML = ReqDetails[0].first_name + " "+ReqDetails[0].last_name;
...
function SaveHRRequest() {
    ...
    if (trainingCalandarArray.length > 0) {
        trainingCalandarArray[0].Location = $('#txtlocation_1').val();
            ...
            document.getElementById('sptblSelectTrainingCalendarList').style.display = 'none';
            CommonAjaxMethod(virtualPath + 'HRRequest/ClubbedTrainingRequest', obj, 'POST', function (response) {
...
function ViewTrainingCalendraList() {
    CommonAjaxMethod(virtualPath + 'HRRequest/GetAllTrainingCalendar', { id: 1 }, 'GET', function (response) {
        var dataArry = response.data.data.Table;
        trainingCalandarArray = response.data.data.Table1;
        //if (trainingCalandarArray.length > 0) {
        //    TrainingTypeID = trainingCalandarArray[0].TrainingTypeid
        //    TrainingName = trainingCalandarArray[0].TrainingType
        //}
        ...
        var newtbblDataScreen = '<table class="table mt-2 " >' +
            ...
            ' <th width="150">Training Name</th>' +
            ' <th width="150" >Training Mode</th>' +
            ' <th width="150" ><span id="spLocationAndLink">Location</span></th>' +
            ' <th style="display:none"></th>' +
            ' </tr>' +
            ' </thead>';
        var htmlScreen = "</table>";
        ...
        for (var i = 0; i < trainingCalandarArray.length; i++) {
            ...
            $('#ddlTrainingMode_' + i).val(trainingCalandarArray[i].TrainingMode == "TrainingMode" ? 143 : 145)
        }
    });
}
function HtmlPaging() {
...
function AddTrainingAttendeesList() {
    ...
    if (valid == true) {
        ...
        for (let i = 0; i < attendeesArray.length; i++) {
            var AttendeesTypeName = attendeesArray[i].AttendeesType == 148 ? 'Mandatory' : attendeesArray[i].AttendeesType == 149 ? 'Optional' : '';
            var RequestID = attendeesArray[i].RequestName == undefined ? 'NA' : attendeesArray[i].RequestName;
            if (attendeesArray[i].RequestName == 'NA') {
                var newtbblData = "<tr><td>" + parseInt(i + 1) + "</td><td>" + attendeesArray[i].AttendeesNames + "</td><td>" + AttendeesTypeName + "</td><td>" + attendeesArray[i].RequestSource + " </td><td>" + RequestID + " </td><td style='display: none'>" + attendeesArray[i].AttendeesType + "</td><td><a  title='Click to Remove' onclick='DeleteAttendeesRows(this," + attendeesArray[i].RowNum + ")' class=\"red-clr\"><i class=\"fas fa-trash-alt\"></i></a></td><td style='display: none'>" + attendeesArray[i].AttendEmployeeID + "</td></tr>";
            }
            else {
                var newtbblData = "<tr><td>" + parseInt(i + 1) + "</td><td>" + attendeesArray[i].AttendeesNames + "</td><td>" + AttendeesTypeName + "</td><td>" + attendeesArray[i].RequestSource + " </td><td><a href='#' data-toggle='modal' data-target='#RequestDetailsModal' onclick=\"ViewRequestDetails('" + attendeesArray[i].ReqID + "')\"><i class='fas fa - eye'></i>" + RequestID + "</a> </td><td style='display: none'>" + attendeesArray[i].AttendeesType + "</td><td></td></tr>";
            }
            var allstring = newtbblData;
            tableData += allstring;
        }
    ...
    if (selected.length > 0) {
        ...
        if (trainingCalandarArray.length > 0) {
            for (i; i < trainingCalandarArray.length; i++) {
                ...
                //}
                //else {
                //    var newtbblData = "<tr><td>" + parseInt(i + 1) + "</td><td>" + trainingCalandarArray[i].TrainingType + "</td><td>" + trainingCalandarArray[i].TrainingName + "</td><td><select onchange=\"ChangedTrainingMode(this," + i + ")\" id='ddlTrainingMode_" + i + "' class=\"form-control dpselect \" tabindex=\"-1\" aria-hidden=\"true\"></select> </td><td><input type='text' id='txtlocation_" + parseInt(i + 1) + "' value='" + dataTrainingRequestCalendar[i].Location + "'></input></td><td style='display: none'>" + trainingCalandarArray[i].TrainingTypeID + "</td></tr>";
                //}
                var newtbblData = "<tr><td>" + parseInt(i + 1) + "</td><td>" + trainingCalandarArray[i].TrainingType + "</td><td>" + trainingCalandarArray[i].TrainingName + "</td><td><select onchange=\"ChangedTrainingMode(this," + i + ")\" id=\"ddlTrainingMode_" + i + "\" class=\"form-control dpselect \" tabindex=\"-1\" aria-hidden=\"true\"></select> </td><td><input type='text' id='txtlocation_" + parseInt(i + 1) + "' value='" + trainingCalandarArray[i].Location + "'></input></td><td style='display: none'>" + trainingCalandarArray[i].TrainingTypeID + "</td></tr>";
                var allstring = newtbblData;
                tableData += allstring;
...
function DeleteAttendeesRows(obj, id) {
    ConfirmMsgBox("Are you sure want to delete", '', function () {
        var data = attendeesArray.filter(function (itemParent) { return (itemParent.RowNum == id); });
        $(obj).closest('tr').remove();
        attendeesArray = attendeesArray.filter(function (itemParent) { return (itemParent.RowNum != id); });
        ReBuildAfterDeleteAttendeesArray()
    })
}
function ReBuildAfterDeleteAttendeesArray() {
    $('#tblAddAttendeesList').html('');
    var newtbblData1 = '<table class="table mt-0 " >' +
        ' <thead>' +
        ' <tr>' +
    ...
    for (let i = 0; i < attendeesArray.length; i++) {
        ...
        if (attendeesArray[i].RequestName == 'NA') {
            attendeesArray[i].RowNum = parseInt(i + 1);
            var newtbblData = "<tr><td>" + attendeesArray[i].RowNum + "</td><td>" + attendeesArray[i].AttendeesNames + "</td><td>" + AttendeesTypeName + "</td><td>" + attendeesArray[i].RequestSource + " </td><td>" + RequestID + " </td><td style='display: none'>" + attendeesArray[i].AttendeesType + "</td><td><a  title='Click to Remove' onclick='DeleteAttendeesRows(this," + attendeesArray[i].RowNum + ")' class=\"red-clr\"><i class=\"fas fa-trash-alt\"></i></a></td><td style='display: none'>" + attendeesArray[i].AttendEmployeeID + "</td></tr>";
        }
        else {
            attendeesArray[i].RowNum = parseInt(i + 1);
            var newtbblData = "<tr><td>" + attendeesArray[i].RowNum + "</td><td>" + attendeesArray[i].AttendeesNames + "</td><td>" + AttendeesTypeName + "</td><td>" + attendeesArray[i].RequestSource + " </td><td><a href='#' data-toggle='modal' data-target='#RequestDetailsModal' onclick=\"ViewRequestDetails('" + attendeesArray[i].ReqID + "')\"><i class='fas fa - eye'></i>" + RequestID + "</a> </td><td style='display: none'>" + attendeesArray[i].AttendeesType + "</td><td></td></tr>";
