...
function ViewTrainingCalendraList() {
    CommonAjaxMethod(virtualPath + 'HRRequest/GetByIDTrainingCalendar', { RequId: ReqID, inputData: 3 }, 'GET', function (response) {
        var dataArry = response.data.data.Table;
        $('#tblTrainingCalendar').html('');
...
function AddTrainingAttendeesList() {
    ...
    if (valid == true) {
        ...
        for (let i = 0; i < attendeesArray.length; i++) {
            var newtbblData = "<tr><td>" + parseInt(i + 1) + "</td><td>" + attendeesArray[i].AttendeesNames + "</td><td>" + attendeesArray[i].AttendeesTypeName + "</td><td>" + attendeesArray[i].RequestSource + " </td><td><a href='#' data-toggle='modal' data-target='#RequestDetailsModal' onclick=\"ViewRequestDetails('" + attendeesArray[i].ReqID + "')\"><i class='fas fa - eye'></i>" + attendeesArray[i].RequestName + "</a>" + attendeesArray[i].RequestName + " </td><td style='display: none'>" + attendeesArray[i].AttendeesType + "</td></tr>";
            var allstring = newtbblData;
            tableData += allstring;
        }
