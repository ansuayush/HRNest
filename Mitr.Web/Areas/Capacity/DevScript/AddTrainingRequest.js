    ...
    $('#ddltrainingName').on('change', function () {
        ...
        CommonAjaxMethod(virtualPath + 'CapacityRequest/GetTrainingRequest', { id: trainingId, inputData: 2 }, 'GET', function (response) {
            ...
            if (response.data.data.Table.length > 0) {
                var TrainingMode = 0;
                document.getElementById('lblTrainingDesc').innerText = response.data.data.Table[0].TrainingDesc;
    ...
    if (checkValidationOnSubmit('Mandatory') == true) {
        ...
        if (isValid) {
            ...
            var HrRequest = {
                ...
                TrainingDescription: document.getElementById('lblTrainingDesc').innerText,
                CreatedBy: loggedinUserid,
                ModifiedBy: loggedinUserid,
                IPAddress: IPAddress,
                HrOrUser: 'User',
