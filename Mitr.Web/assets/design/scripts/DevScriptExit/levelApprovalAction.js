...
function BindScreen() {
   ...
   debugger
        ...
        if (response.data.data.Table2.length > 0) {
            if (LevelForm == "1") {
                item = $(response.data.data.Table2).filter(function (index, row) {
                    return row.DocType === "L1" ;
                });
            }
            else if (LevelForm == "2") {
                item = $(response.data.data.Table2).filter(function (index, row) {
                    return row.DocType === "L1" || row.DocType === "L2";
                });
          
            }
            else if (LevelForm == "3") {
                item = $(response.data.data.Table2).filter(function (index, row) {
                    return row.DocType === "L1" || row.DocType === "L2" || row.DocType === "L3";
                });
             
            }
            else if (LevelForm == "4") {
                item = $(response.data.data.Table2).filter(function (index, row) {
                    return row.DocType === "L1" || row.DocType === "L2" || row.DocType === "L3" || row.DocType === "L4";
                });
           
            }
            else if (LevelForm == "5") {
                item = $(response.data.data.Table2).filter(function (index, row) {
                    return row.DocType === "L1" || row.DocType === "L2" || row.DocType === "L3" || row.DocType === "L4" || row.DocType === "L5";
                });
