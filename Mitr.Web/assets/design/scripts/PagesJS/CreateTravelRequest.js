...
function DeleteProjectItemList(obj) {
    ...
    var MapProjectID = parseInt($(obj).closest('tr').find("td.hfmapprojectId input[type='hidden']").val());
    var proReqDet_ID = parseInt($(obj).closest('tr').find("td.hfproReqdetid input[type='hidden']").val());
    var count = 0;
    var TotalRowCount = $('#tblProjects').find("tbody tr").length;
    if (isNaN(MapProjectID)) {
        ...
        ConfirmMsgBox("Are you sure want to delete", '', function () {
            ...
            var dataObject = JSON.stringify({
                'ID': MapProjectID,
                'AdditionalID': proReqDet_ID,
                'Doctype':'TravelSublineItemRemove',
            });
...
function FillProjectgrid() {
    ...
    if (DPro == "") {
        ...
        if (!CombinationExist) {
            ...
            $("#ddProjectDropdown")[0].selectedIndex = 0;
            $('#ddProjectDropdown').val(0).trigger('change');
            $("#ddProjectDetailDropdown").empty();
            $("#ddProjectDetailDropdown").append($("<option     />").val("").text("Select"));
           ShowHideNewAddProject();
