...
function BindTarget(args) {
	...
	CloseLoadingDialog();
	var LastTRCount = parseInt($('#tblActivity').find("tbody tr").length);
	if (LastTRCount > 4) {
		const element = document.getElementById("tblActivity");
...
function ValidateVerfiyAndSave() {
	...
	var PendingDates = $("#LeaveWithAttachmentPendingDates").val();
	...
	if (a > 0) {
		...
		var content = 'Please submit Medical/ Fitness certificate against leave request number(s) ' + PendingDates;
		...
		function funCan() {
			var ms1 = "The leave availed against Cl/Sl shall be converted to Annual Leave/ LWP, as the case may be, pl confirm if you want to proceed.";
			SyncCloseDialog();
	...
	{
		SyncConfirmBox('Warning', "You are entitled for compensatory off, do you want to apply?", funCoOk, funCoCan)
		function funCoOk() {
			SyncCloseDialog();
...
function ValidateDays() {
	...
	if (workinghours == 0) {
		Msg = "Working Hours is not set in Salary Structure"
	}
	else
	{
		$(".validatehours").each(function (index) {
			var a = parseInt(Number($(this).html()));
			if (a < workinghours || a > 24) {
	...
	$.ajax({
		...
		success: function (data) {
			...
			$("#btnAddSaveProject").click(function () {
				...
				debugger;
				$("#btnclose").click();
		       // $("#adproject").modal('hide');
				//$("#adproject").removeClass('show');
			});
		}
	});
}
function GetAddProjectListNew() {
...
function AfterOnSuccess(obj) {
	...
	if (obj.Status) {
		if (obj.StatusCode == 0 && obj.OtherID==0) {
			SuccessToaster(obj.SuccessMessage);
           window.location = obj.RedirectURL;
		}
      else{
        GetAddProjectListNew();
       CloseLoadingDialog();
     }
...
function fnRecall() {
	swal("Write Remarks", {
		text: "Please state reason to recall the activity log",
		content: "input",
